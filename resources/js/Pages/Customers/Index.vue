...
function deleteCustomer(id) {
    if (confirm('මෙම පාරිභෝගිකයා මකා දැමීමට ඔබට විශ්වාසද?')) {
...
<template>
    <Head title="පාරිභෝගිකයන්" />

    <AuthenticatedLayout>
        <template #header>
            <h1 class="text-xl font-bold text-gray-800">පාරිභෝගිකයන්</h1>
        </template>
        ...
        <div class="flex flex-col sm:flex-row gap-3 mb-4">
            <div class="flex-1 relative">
                ...
                <input
                    v-model="search"
                    type="text"
                    placeholder="පාරිභෝගිකයා සොයන්න..."
            ...
            >
                <div class="flex justify-between items-start mb-2">
                    ...
                    <div class="text-right">
                        <p class="text-xs text-gray-400">ණය ශේෂය</p>
            ...
            <table class="w-full">
                <thead>
                    <tr class="text-left text-xs font-semibold text-gray-500 uppercase tracking-wider bg-gray-50 border-b border-gray-100">
                        ...
                        <th class="px-4 py-3">ණය සීමාව</th>
                        <th class="px-4 py-3">ණය ශේෂය</th>
                        <th class="px-4 py-3 text-right">ක්‍රියා</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-gray-100">
                    <tr v-if="customers.data?.length === 0">
                        <td colspan="6" class="px-4 py-8 text-center text-gray-400">පාරිභෝගිකයන් නොමැත</td>
                    </tr>
                    <tr
                        v-for="customer in customers.data"
                        :key="customer.id"
